...
<?php
   ...
   if(!empty($start)|| !empty($end)||!empty($name)||!empty($room)){ 
       if(($adultN+$childrenN) <=0 || $roomN<=0){
            $message = "You suppose to select the number of room, and the number of adult and/or children.";
            $_SESSION["error"]=$message;
            header('Location:../index.php');  
            exit(); 
       }elseif(!$typeR =="Double Room" || !$typeR=="Single Room" || !$typeR=="Special Room"){
            $_SESSION['id'] = "You suppose to select the type of room.  ";
            $_SESSION["error"]=$message;
            header('Location:../index.php');  
            exit();
       }elseif(!($dateTimeObject1 && $dateTimeObject2)){
            $message = "Please insert a valid date, one of them is not valid ".$end." , ".$start;
            $_SESSION["error"]=$message;
            header('Location:../index.php');  
            exit(); 
